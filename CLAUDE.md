...
- Internal plugins: this repo = plugin `ccs` + marketplace `minh-internal` (`.claude-plugin/`); `rcp-engine` = git-subdir of nmt-tool-remote-control-pc `plugin/`@main. `ccs` installs from its GIT URL — NEVER switch it to a path source (a path install copies the gitignored env secrets into the plugin cache). Plugin-visible change (`skills/`, `hooks/`) → bump `.claude-plugin/plugin.json` version, commit, push (git-sourced installs read origin). CD: `bin/plugin-autoupdate.sh` + `systemd/ccs-plugin-cd.*` user timer; manual: `--force`.
</dev_tasks>
